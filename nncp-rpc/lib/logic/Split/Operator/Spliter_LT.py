...
class Spliter(Spliter_Common.Spliter):
    ...
    def splitCodes_PT(self, prjInfo):
        ...
        for item in ccArray:
            ...
            # 红球注数
            r_zhushu = self.ArrInfo[len(rArr)][5]
            # 篮球注数
            b_zhushu = self.ArrInfo[len(bArr)][2]
            ...
            if r_zhushu > self.limitMoney / self.singleMoney:
                # 1 拆前区前18球组合
                ret_h = self.handFushi(rArr[0:18], bArr, beishu)
                ret += ret_h

                # 2 拆前区后17个球的组合
                if len(rArr[18:]) > 4:
                    ret_h = self.handFushi(rArr[18:], bArr, beishu)
                    ret += ret_h
    ...
    def splitRedBall(self, rArr):
        ...
        for i in range(1, max + 1):
            ...
            for a in Math.combination(r1, 5 - i):
                for b in tmp:
                    yield a + b
    ...
    def handFushi(self, rArr, bArr, beishu):
        ...
        if zhushu * self.singleMoney * max_beishu > self.limitMoney:
            ...
            # 需要拆蓝球
            if lmt_beishu == 0:
                tmpArr = Math.combination(bArr, 2)

                for info in tmpArr:
                    info_zhushu = self.ArrInfo[len(rArr)][5] * 1
    ...
    def splitCodes_DT(self, prjInfo):
        ...
        for item in ccArray:
            ...
            # 篮球胆码
            blue_d = m.group(3).split(',')

            # 篮球拖码
            blue_t = m.group(4).split(',')
            ...
            # 红球注数
            r_zhushu = self.ArrInfo[len(red_t)][5 - len(red_d)]

            # 篮球注数
            b_zhushu = self.ArrInfo[len(blue_t)][2 - len(blue_d)]
            zhushu = r_zhushu * b_zhushu
...
if __name__ == '__main__':
    # args = {'fileorcode':'05,09,27,28,32|07,11$05,10,26,27,32|07,11$05,11,27,28,30|07,11$06,09,27,28,30|07,11$05,13,26,27,30|07,11$06,10,11,25,32|05,06$06,13,27,28,30|05,06$07,09,10,26,32|05,06$07,10,11,28,30|05,06$07,13,25,26,32|05,06$01,02,03,04,05,06,07|01,02,03', 'beishu':'1', 'wtype':'1'}
    # args = {'fileorcode':'01,02,03,04,05,06,07,08,09,10,11,12,13,14,15,16,17,18|01,02,03', 'beishu':'2', 'wtype':'1'}
    args = {
        'fileorcode': '[D:01][T:02,03,04,05,06,07,08,09,10,11,12,13,14,15,16,17]|[D:01][T:02,03]$[D:01,02][T:03,04,05,06]|[D:01][T:05,06]',
        'beishu': '3', 'wtype': '135'}
    obj = Spliter(28)
