...
from commonEntity.Jl import JclqBean
logger = Log().getLog()


@ews.route_sync_func('/jl/trade', kwargs={'ck': (UserWarning, ),
...
def trade(handler, *args, **kwargs):
    ...
    firsttime, lasttime = JclqBean().get_firsttime_and_lasttime(fileorcode)
    params.update({
        "uid": uid,
        "firsttime": firsttime,
        "lasttime": lasttime
...
@ews.route_sync_func('/jl/matches')
def jl_matches(handler, *args, **kwargs):
    platform = handler.json_args.get("platform", "")
    ret = JclqBean().get_on_sale_matches(platform)
    return handler.ok(ret)


@ews.route_sync_func('/jl/follow', kwargs={'ck': (UserWarning,),
                                           'lotid': (UserWarning, 'unsigned int'),
                                           'beishu': (UserWarning, 'unsigned int'),
                                           'allmoney': (UserWarning, 'unsigned int'),
                                           'pid': (UserWarning, 'int')})
