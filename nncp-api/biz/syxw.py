...
@ews.route_sync_func('/syxw/issue')
def gx11x5_issue(handler, *args, **kwargs):
    lotid = handler.json_args.get("lotid", "")
    ret = SyxwBean(lotid).get_syxw_expect_list()
    return handler.ok(ret)


@ews.route_sync_func('/syxw/trade', kwargs={'ck': (UserWarning,),
